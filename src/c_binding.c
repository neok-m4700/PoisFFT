...
#define SOLVER(dims, suff) typedef void *poisfft_solver ## dims ## d ## suff;
...
#define POIS_FINALIZE(dims, suff)   \
    extern void poisfft_solver ## dims ## d ## suff ## _finalize(poisfft_solver ## dims ## d ## suff *D);
...
poisfft_solver poisfft_solver_new(const int dims, const int *nxyz, const double *Lxyz, const int *BCs, const int approximation,
                                  const int *gnxyz, const int *offs, const void *mpi_comm, int nthreads)
{
  ...
  {
  case 1:
    poisfft_solver1d_new((poisfft_solver1d *) & (S.D), nxyz, Lxyz, BCs, &approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  case 2:
    poisfft_solver2d_new((poisfft_solver2d *) & (S.D), nxyz, Lxyz, BCs, &approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  case 3:
    poisfft_solver3d_new((poisfft_solver3d *) & (S.D), nxyz, Lxyz, BCs, &approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  default:
...
void poisfft_solver_f_new(const int dims, const int *nxyz, const float *Lxyz, const int *BCs, const int *approximation,
                          const int *gnxyz, const int *offs, const void *mpi_comm, int nthreads)
{
  poisfft_solver_f S;
  S.dims = dims;
  ...
  {
  case 1:
    poisfft_solver1d_f_new((poisfft_solver1d_f *) & (S.D), nxyz, Lxyz, BCs, approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  case 2:
    poisfft_solver2d_f_new((poisfft_solver2d_f *) & (S.D), nxyz, Lxyz, BCs, approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  case 3:
    poisfft_solver3d_f_new((poisfft_solver3d_f *) & (S.D), nxyz, Lxyz, BCs, approximation, gnxyz, offs, mpi_comm, nthreads);
    break;
  default:
    printf("Error, PoisFFT solver dimension must be 1, 2 or 3!\n");
    break;
