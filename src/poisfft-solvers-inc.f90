subroutine PoisFFT_Solver1D_FullPeriodic(D, Phi, RHS)
   ...
   if (D % mpi_transpose_needed) then
      ...
      call MPI_Gather(D % offx, 1, MPI_INTEGER, displs, 1, MPI_INTEGER, 0, D % mpi % comm, ie)

      call MPI_Gather(D % nx, 1, MPI_INTEGER, counts, 1, MPI_INTEGER, 0, D % mpi % comm, ie)

      call MPI_Gatherv(RHS, D % nx, MPI_RP, tmp, counts, displs, MPI_RP, 0, D % mpi % comm, ie)
      ...
      if (D % mpi % rank == 0) then
         ...
         forall(i=2:D % gnx) D % cwork(i) = D % cwork(i) / D % denomx(i)
         call Execute(D % backward, D % cwork)
      end if

      tmp = real(D % cwork, RP) / D % norm_factor

      call MPI_Scatterv(tmp, counts, displs, MPI_RP, Phi, D % nx, MPI_RP, 0, D % mpi % comm, ie)
   end if
#else
   ! Forward FFT of RHS
   D % cwork = cmplx(RHS, 0._RP, CP)
   call Execute(D % forward, D % cwork)
   forall(i=2:D % nx) D % cwork(i) = D % cwork(i) / D % denomx(i)
   call Execute(D % backward, D % cwork)
...
subroutine PoisFFT_Solver1D_FullDirichlet(D, Phi, RHS)
   ...
   forall(i=1:D % nx) D % rwork(i) = D % rwork(i) / D % denomx(i)
...
subroutine PoisFFT_Solver1D_FullNeumann(D, Phi, RHS)
   ...
   call Execute(D % forward, D % rwork)
   D % rwork(1) = 0
   forall(i=2:D % nx) D % rwork(i) = D % rwork(i) / D % denomx(i)
   call Execute(D % backward, D % rwork)
   Phi = D % rwork / D % norm_factor
end subroutine
   ...
   if (D % ny < D % gny) then
      ...
      do k = 1, D % nz
         ...
         if (k == 1 .and. D % offz == 0) then
            do j = 2, D % ny
               do i = 2, D % nx
                  D % Solvers2D(tid) % cwork(i, j) = D % Solvers2D(tid) % cwork(i, j) / (D % denomx(i) + D % denomy(j))
               end do
            end do
            do i = 2, D % nx
               D % Solvers2D(tid) % cwork(i, 1) = D % Solvers2D(tid) % cwork(i, 1) / (D % denomx(i))
            end do
            do j = 2, D % ny
               D % Solvers2D(tid) % cwork(1, j) = D % Solvers2D(tid) % cwork(1, j) / (D % denomy(j))
            end do
            !NOTE: if IEEE FPE exceptions are disabled all this is not necessary and
            ! the loop can be over all indexes because the infinity or NaN is changed to 0 below
            ...
            do j = 1, D % ny
               do i = 1, D % nx
                  D % Solvers2D(tid) % cwork(i, j) = D % Solvers2D(tid) % cwork(i, j) / (D % denomx(i) + D % denomy(j) + D % denomz(k))
               end do
            end do
...
subroutine PoisFFT_Solver3D_PNsP(D, Phi, RHS)
   ...
   do j = 1, D % ny
      ...
      if (j == 1) then
         do k = 2, D % nz
            do i = 2, D % nx
               D % Solvers2D(tid) % cwork(i, k) = D % Solvers2D(tid) % cwork(i, k) / (D % denomx(i) + D % denomz(k))
            end do
         end do
         do i = 2, D % nx
            D % Solvers2D(tid) % cwork(i, 1) = D % Solvers2D(tid) % cwork(i, 1) / (D % denomx(i))
         ...
         do k = 2, D % nz
            D % Solvers2D(tid) % cwork(1, k) = D % Solvers2D(tid) % cwork(1, k) / (D % denomz(k))
         end do
         !NOTE: if IEEE FPE exceptions are disabled all this is not necessary and
         ! the loop can be over all indexes because the infinity or NaN is changed to 0 below
         D % Solvers2D(tid) % cwork(1, 1) = 0
         ...
         do k = 1, D % nz
            do i = 1, D % nx
               D % Solvers2D(tid) % cwork(i, k) = D % Solvers2D(tid) % cwork(i, k) / (D % denomx(i) + D % denomy(j) + D % denomz(k))
            end do
         end do
      endif
      call Execute(D % Solvers2D(tid) % backward, D % Solvers2D(tid) % cwork)
